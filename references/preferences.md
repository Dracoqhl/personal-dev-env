...
- Codex skill, plugin, and MCP additions should be recorded by default after successful installation and verification. Record skill names, source repositories or local paths, restore paths, MCP command shapes, and environment variable names; never record API key values, provider tokens, auth files, or local Codex databases.
